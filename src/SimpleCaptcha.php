...
class SimpleCaptcha
{
    ...
    /**
     * Blur effect for better image quality (but slower image
     * processing).
    ...
    public function CreateImage()
    {
        ...
        if ($this->debug)
        {
            imagestring($this->im,
                ...
                1,
                $this->height - 8,
                "$text {$fontcfg['font']} " . round(
                    (microtime(true) - $ini) * 1000) . "ms",
                $this->GdFgColor);
        }
    ...
    protected function ImageAllocate()
    {
        ...
        $this->GdFgColor = imagecolorallocate($this->im,
            $color[0],
            $color[1],
            $color[2]);

        // Shadow color
        if (!empty($this->shadowColor) && is_array($this->shadowColor) && sizeof(
            $this->shadowColor) >= 3)
        {
    ...
    protected function WriteLine()
    {
        ...
        $x2 = $this->textFinalX;
        $y1 = rand($this->height * $this->scale * .40,
            $this->height * $this->scale * .65);
        $y2 = rand($this->height * $this->scale * .40,
            $this->height * $this->scale * .65);
    ...
    protected function WriteText($text, $fontcfg = array())
    {
        ...
        $fontpath = $this->resourcesPath . '/fonts/' . $fontcfg['font'];
        $fontfile = stream_resolve_include_path($fontpath);

        if ($fontfile === false)
        {
            throw new \RuntimeException(
                sprintf(
                    'Could not find/open font file "%s", include path "%s"',
                    $fontpath,
                    get_include_path()));
        }
        ...
        for ($i = 0; $i < $length; $i++)
        {
            $degree = rand($this->maxRotation * -1,
                $this->maxRotation);
    ...
    protected function ReduceImage()
    {
        ...
        $imResampled = imagecreatetruecolor($this->width,
            $this->height);
        imagecopyresampled($imResampled,
            $this->im,
            0,
            0,
            0,
